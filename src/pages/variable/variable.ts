...
export class VariablePage {
  ...
  public bpm: any;                     // Contains the value of our cloud variable
  ...
  ionViewDidEnter() {
    // When entering the page, subscribe to updates to the Particle cloud varibale bpm
    if (this.particle.device) {
        this.cancelSubscription();
        this.subscription = this.particle.pollVariable("bpm").subscribe(
            (value) => { this.bpm = value; this.subscribed = true; },
            (error) => { console.log("Error reading bpm"); this.subscribed = false; },
            () => { console.log("Stopped polling bpm"); this.subscribed = false; }
        );
    }
  }
